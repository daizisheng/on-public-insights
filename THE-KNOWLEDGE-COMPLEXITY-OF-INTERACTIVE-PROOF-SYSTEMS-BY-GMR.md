...
> 
> 对于阶为素数 $p$ 的循环群 $G$ ，令其生成元为 $g$ 。假设这个群上的离散对数问题是难解的。输入为 $x,n=|x|=\lceil \log{p} \rceil$ 。随机变量 $U(x)$ 对所有的 $g^{0,1,...,2^{n-1}-1}$ 赋予相同概率 $1/2^{n-1})$ 。随机变量 $V(x)$ 则对所有的 $g^{n/2,...,p-1}$ 赋予相同概率 $1/(p-n/2)$ 。
>
