...
|B'的新数据 $y=pk+r$ | 0.5 | 0.5
>
>（16）实际上，就本例来说，似乎要求随机变量的分布是一样的似乎并没有必要。为了说明这一点，我们来看另外一个例子。

### 注：一个传递了知识的例子

>（17）稍微修改一下上面的(A,B')交互系统。A的策略进行修改，它选择 $r$ 的范围是 $`\{0,...,\frac{n}{2}-1\}`$ （这里我们假设 $n$ 是偶数）。如果我们取 $n=4$ ，那么 $r$ 的取值范围就是 $`\{0,1\}`$ 。B'在得到了 $y=pk+r \mod n$ 之后，可以把 $pk$ 的可能范围从 $`\{0,...,n-1\}`$ 缩小为 $`\{y,y-1,...,y-\frac{n}{2}+1\} \mod n`$ ，也就是 $pk$ 的可能范围缩小了一半。举例来说，如果 $y=3$ ， 则交互之后，B'对 $pk$ 的可能范围缩小为了 $`\{2,3\}`$ 。容易看出，这个过程中B'是肯定学到了知识的，而且还是我们 **真正关注的那种知识** 。
>
>（18）现在我们来看第三方C。首先，C对 $pk$ 一无所知。C可以通过读取 $r$ 来输出一个 $y'$ 给B'，我们不妨还是按照前面的思路来做，仍然输出 $y'=r$ 。依然分类讨论（注意 $n=4$ ）：
>
> 令 $pk=0$
>
| $r$ 取值 |  概率   |  B'和A交互所得 | 从A得到知识  | C产生数据 | C自己计算知识
|---------|:-------:|-------------:|------------:|----------:|------------
| 0       |   0.5   | $y=0$        | $Z(addr,0)$ | $y'=r=0$  | $Z(addr,0)$
| 1       |   0.5   | $y=1$        | $Z(addr,1)$ | $y'=r=1$  | $Z(addr,1)$
>
> 令 $pk=1$
>
| $r$ 取值 |  概率   |  B'和A交互所得 | 从A得到知识  | C产生数据 | C自己计算知识
|---------|:-------:|-------------:|------------:|----------:|------------
| 0       |   0.5   | $y=1$        | $Z(addr,1)$ | $y'=r=0$  | $Z(addr,0)$
| 1       |   0.5   | $y=2$        | $Z(addr,2)$ | $y'=r=1$  | $Z(addr,1)$
>
> 令 $pk=2$
>
| $r$ 取值 |  概率   |  B'和A交互所得 | 从A得到知识  | C产生数据 | C自己计算知识
|---------|:-------:|-------------:|------------:|----------:|------------
| 0       |   0.5   | $y=2$        | $Z(addr,2)$ | $y'=r=0$  | $Z(addr,0)$
| 1       |   0.5   | $y=3$        | $Z(addr,3)$ | $y'=r=1$  | $Z(addr,1)$
>
> 令 $pk=3$
>
| $r$ 取值 |  概率   |  B'和A交互所得 | 从A得到知识  | C产生数据 | C自己计算知识
|---------|:-------:|-------------:|------------:|----------:|------------
| 0       |   0.5   | $y=3$        | $Z(addr,3)$ | $y'=r=0$  | $Z(addr,0)$
| 1       |   0.5   | $y=0$        | $Z(addr,0)$ | $y'=r=1$  | $Z(addr,1)$
