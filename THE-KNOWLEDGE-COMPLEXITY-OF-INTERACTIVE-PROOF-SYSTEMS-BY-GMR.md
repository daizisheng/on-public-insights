...
### 注
> 我们来考察一种极端情况，即 $`c = +\infty`$ 。上面的公式变成:
> $$ \displaystyle\sum_{\alpha \in \{0,1\}^*} | prob( U(x) = \alpha) - prob(V(x) = \alpha)| = 0$$
> 等价于，对任意的 $`\alpha \in \{0,1\}^*`$，有：
> $$ prob( U(x) = \alpha) = prob(V(x) = \alpha)$$
> 这应该就是把 **统计不可区分** 极端化成为了 **相等** ，即两个随机变量 $U(x)$ 和 $V(x)$ 的分布完全相同。
> 
