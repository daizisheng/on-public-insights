...
**定义**：设M是一个概率图灵机，对于输入 $x$ 以概率1停机。我们用 $M(x)$ 表示这样的随机变量：对于每个字符串 $a$ ，它以与M在输入 $x$ 上输出 $a$ 的概率完全相同的概率取值 $a$ 。

**定义**：设 $`L \subset \{0,1\}^*`$ 是一个语言，$`U=\{U(x)\}`$ 是一个随机变量族。如果存在一个期望多项式时间运行的概率图灵机M，使得对于所有 $`x\in L`$ ， $M(x)$ 等于 $U(x)$ ，则我们称 $U$ 在 $L$ 上是 **完美可近似** 的。如果存在一个期望多项式时间运行的概率图灵机M，使得随机变量族 $`\{M(x)\}`$ 和 $`\{U(x)\}`$ 在 $L$ 上是统计（计算）不可区分的，则我们称 $U$ 在 $L$ 上是 **统计（计算）可近似** 的。
...
如果对于所有的概率多项式时间ITM B'，(A,B')在L上对于所有的B'都是完美的（统计上的）（计算上的）零知识，那么我们说（A, B）在 $L$ 上是完美的（统计上的）（计算上的）零知识。

需要注意的是，（A, B）对于B'的零知识定义（以某种方式）仅取决于A，而与B无关。更准确地说，可以将A视为对于B'的零知识。在2.2中，交互式证明系统的定义中也存在类似的问题。定义的第二部分仅取决于B，而与A无关。

计算零知识无疑是上述概念中最通用的，我们将简称为零知识。零知识真正捕捉到了关于 $L$ 的成员的任何信息，这些信息在多项式时间内无法高效获取。也就是说，如果（A，B）是零知识的，那么在概率多项式时间内，通过与A进行交互，甚至通过"作弊"也无法提取关于L的任何信息。

**定义** 设 $`L \subset \{0，1\}^*`$ 是一个语言。如果（A，B）是 $L$ 的一个完美的（统计的）（计算的）零知识证明系统，那么它是 $L$ 的一个交互证明系统，并且是在 $L$ 上完美的（统计的）（计算的）零知识协议。

我们将简单地将计算零知识证明系统（三个概念中最通用的概念）称为零知识证明系统。在实际世界中，这个概念是完全适用的。也就是说，如果（A，B）是 $L$ 的零知识证明系统，那么即使对于一个“作弊”的B'来说，在多项式时间内与A进行交互也无法提取出除了 $L$ 成员证明之外的任何其他信息。
