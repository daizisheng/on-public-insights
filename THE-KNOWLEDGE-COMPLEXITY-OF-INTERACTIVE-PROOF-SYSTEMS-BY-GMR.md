...
**定义**：设 $`L \in \{0,1\}^*`$ 是一个语言，$`U=\{U(x)\}`$ 是一个随机变量族。如果存在一个期望多项式时间运行的概率图灵机M，使得对于所有 $`x\in L`$ ，$M(x)$ 等于 $U(x)$ ，则我们称 $U$ 在 $L$ 上是 **完美可近似** 的。如果存在一个期望多项式时间运行的概率图灵机M，使得随机变量族 $`\{M(x)\}`$ 和 $`\{U(x)\}`$ 在 $L$ 上是统计（计算）不可区分的，则我们称 $U$ 在 $L$ 上是 **统计（计算）可近似** 的。
