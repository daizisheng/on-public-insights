...
>
> 如果上面的算法C存在，转变为电路C'，使得 $`C(s)=x \iff C'(s)=1`$ 。那么我们就有 $`|P(s_0,C',0) - P(s_1,C',0)| > 1/2 + |x|^{-c_0} - (1/2 - |x|^{-c_1}) = |x|^{-c_0} + |x|^{-c_1} > |x|^{-\min(c_0,c_1)}`$ 。 对 $x=1$ 是也可以类似说明。这就说明了，算法C的存在，就能够推导出随机变量 $s_0$ 和 $s_1$ 的计算可区分性。由于分布的等同性，我们也就推导出了 $U(x)$ 和 $V(x)$ 的计算可区分性，这个例2的结论是矛盾的。这就说明了，算法C是不存在的。因此，我们可以合理的认为上述过程是 **某种零知识** 的，我们可以定义为某种 **可计算意义下的零知识** 。
>
