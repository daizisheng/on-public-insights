...
>
> 算法针对不同长度的 $|y|$ 有不同的版本，即 $ALGO_{n=|y|}$ 。算法 $ALGO_n$ 拥有一个额外索引存储，上面按照 $key$ 的顺序，依次记录了  $`(key,val) = (key=h(val),val)`$ 。有了这种的辅助输入， $ALGO_n$ 采用二分查找，实际上是可以快速逆转 $y=h(x)$ 的。不过要注意的是，这里的所谓辅助输入的大小是输入 $y$ 的大小的指数级别。如果转换为电路的话，这个非均匀电路也将是输入 $y$ 的大小的指数级别。这个和上面提到的多项式级别的电路有所不同，但均匀和非均匀的意思大致一致。
>
