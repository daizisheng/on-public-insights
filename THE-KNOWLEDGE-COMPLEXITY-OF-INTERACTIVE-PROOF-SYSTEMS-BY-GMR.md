...
首先，让我们强调B的硬币抛掷是零知识定义中视图的重要组成部分。考虑所有合数的语言 $L$ 和以下协议（A，B）。在输入一个整数 $n$ 时，B随机选择一个介于 $1$ 和 $n$ 之间与 $n$ 互质的整数 $x$ 。然后，它向A发送数字 $`a = x^2 \mod n`$ 。A通过发送 $y$ 作为 $a \mod n$ 的一个随机选择的平方根来做出回应。如果视图仅包含A和B之间的交互文本，则上述协议将在 $L$ 上是完全的零知识。然而，我们定义视图以包含B的硬币抛掷。因此，如果在 $View_{(A,B)} (n)$ 中随机选择的 $(x, a, y)$ ，那么 $`gcd(x+y, n)`$ 不是 $1$ 或 $n$ 的概率至少为 $1/2$ 。因此，如果因子分解不在概率多项式时间内可行，则上述协议对于B来说不是零知识。（然而，值得注意的是，在零知识的定义中（对于所有的B'），不需要将B'的随机比特包含在视图中；这是因为我们已经将B'发送的消息包含在视图中，对于每个B'，都存在一个类似B'的B"，只是它将其随机比特作为其最后一个消息的一部分发送。）
