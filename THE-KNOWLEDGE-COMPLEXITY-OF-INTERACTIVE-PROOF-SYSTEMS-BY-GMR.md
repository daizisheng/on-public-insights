...
>
>（14）稍微修改一下这个例子，我们可以得到一个有知识的交互。在修改后的例子中，我们不再随机选择 $r$ ，而不是以 $3/4$ 的概率选择 $r=0$ ，而 $1/4$ 的概率选择 $r=1$ 。仍然套入上面的公式，我们将得到
...
> 不难看出，交互之后，无论 $s=0$ 还是 $s=1$ ，B关于 $x$ 的后验知识已经发生了变化。比如，如果 $x=0$ ，B可以合理合理的判断 $x$ 更有可能是一个偶数。因此，我们应该合理的认为，上面这个交互过程中，**不是零知识的**。
