...
`$$
>
> 这个例子说明了，随机变量统计不可区分的一种情况，也就是两个随机变量只在非常少的情况下有概率上的差别，在别的绝大部分情况下概率都是一样的。比如例子中， $U(x)$ 和 $V(x)$ 只在 $0^{|x|}$ 以及 $1^{|x|}$ 处分别给予了 $0$ 和 $2^{-|x|+1}$ 的概率。我们可以想象，所谓的“裁判”在拿到多项式大小的样本的时候，大概率根本没有触及到这两个点，而只有这两个点才能够真正的把两个随机变量区分开来。
