...
注解2. 让我们指出上述定义的鲁棒性。在这个定义中，我们只给我们计算有界的“判断者”样本大小为 $1$ 。然而，这并不具限制性。我们注意到，如果对于每个独立地根据分布 $U_x$ 生成的输入字符串（数量为 $|x|$ 的多项式），当 $C_x$ 被赋予这些输入时，接受的概率接近于使用 $V_x$ 时的接受概率，那么两个随机变量族 $`\{U_x\}`$ 和 $`\{V_x\}`$ 在计算上是不可区分的（相对于大小为 $1$ 的样本）。
